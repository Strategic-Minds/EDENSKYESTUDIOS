...
const systemCards = [
  ...
  ['Auto Social', 'Drive-fed model lanes, multi-network drafts, Supabase receipts, and approval-controlled scheduling.', 'RT-013'],
...
export default function Home() {
  return (
    <main className="edenSite">
      ...
      <section id="control-plane" className="automationPanel"><p className="welcome">Autonomous System Control Plane</p><h2>Website feeds the system.</h2><p className="panelLead">This site is the operator-facing surface for the Auto Social pipeline. It connects Drive source truth, Supabase-ready records, Git branch builds, Vercel workflow checks, AUTO BUILDER 2 routing, Shopify bridge payloads, and approval-controlled social scheduling.</p><div className="runtimeGrid">{runtimeGrid.map(([item, body]) => <article key={item}><strong>{item}</strong><span>{body}</span></article>)}</div><div className="pipelineStrip">{pipelineSteps.map((step, index) => <span key={step}>{index + 1}. {step}</span>)}</div><div className="controlActions"><a href="/api/eden/auto-social">Test Auto Social Pipeline</a><a href="/api/eden/status">Runtime Status</a><a href="/api/eden/shopify-bridge">Shopify Bridge</a></div></section>
