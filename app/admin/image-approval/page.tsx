...
const sourceImages = [
  {
    title: 'Eden Skye Active Source Portrait',
    fileName: '01-eden-labeled-eden-skye.png',
    driveFileId: '1lmOJBPF0G2wotinP7Q9iDiVqorZI8FQs',
    status: 'Active source for draft generator use',
    usage: 'Single approved identity-lock source for branded website image generation.',
    generatorState: 'active_single_reference_source',
...
export default function ImageApprovalPage() {
  ...
  return (
    <main className="approvalPage">
        ...
        <div className="settingsGrid">
          ...
          <label>
            ...
            <select value={settings.referenceMode} onChange={(event) => updateSetting('referenceMode', event.target.value as RuntimeSettings['referenceMode'])}>
              <option value="on">On - use active Eden source</option>
              <option value="off">Off - text only</option>
            </select>
          </label>
          <label>
            <span>Reference images</span>
            <input type="number" min={1} max={1} value={settings.referenceCount} onChange={(event) => updateSetting('referenceCount', Number(event.target.value) || 1)} />
          </label>
          <label>
      ...
      <section className="imageGrid singleSourceGrid" aria-label="Eden Skye active source image">
