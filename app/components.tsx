import { heroAsset, missingAssets, models, primaryModel, standaloneAssets } from './visual-source-truth';
...
export function ModelCard({ model }: { model: typeof models[number] }) {
  return <a className="model-card" href={`/models/${model.slug}`}><img src={model.image} alt={`${model.name} approved standalone source`} /><span><strong>{model.name}</strong><em>{model.location}</em></span></a>;
}
...
export { heroAsset, models, primaryModel, standaloneAssets };
